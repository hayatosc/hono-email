...
`hono-email/cloudflare-email` provides the REST adapter for Cloudflare Email Service.
For Cloudflare Workers, use the Cloudflare-specific subpath `hono-email/cloudflare-email/cloudflare`.

Workers binding (`Env` is generated by `wrangler types`):
...
- [`examples/cloudflare-vite-tailwind`](./examples/cloudflare-vite-tailwind/README.md)
  - Uses `hono-email/plugin` for `<Tailwind>` artifact injection
  - Sends through `sendEmail({ adapter: WorkersConnector(), ... })`
  - Reads route-level vars with Hono `env(c)` helper
  - Uses generated `worker-configuration.d.ts` from `wrangler types`
