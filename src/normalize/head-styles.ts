const GENERATED_HEAD_STYLE_PATTERN = /<style\b[^>]*data-hono-email-head="true"[^>]*>[\s\S]*?<\/style>/gi
...
export const relocateHeadStyles = (html: string): string => {
  ...
  if (htmlOpen?.index !== undefined) {
    return normalizedHtml.replace(HTML_OPEN_PATTERN, `${htmlOpen[0]}<head>${generatedStyles.join('')}</head>`)
  }

  const bodyOpen = normalizedHtml.match(BODY_OPEN_PATTERN)
  if (bodyOpen?.index !== undefined) {
    return normalizedHtml.replace(BODY_OPEN_PATTERN, `<head>${generatedStyles.join('')}</head>${bodyOpen[0]}`)
  }
