...
By default, `hono-email` validates compatibility against the major email clients:
...
If a feature is completely unsupported (marked as `'n'` in caniemail.com) on any of the target clients, it will throw an error or warning depending on your configuration.

## Warning Clients Configuration

You can customize which email clients to validate against by configuring the `warningClients` option.

```tsx
const result = await render(<MyEmail />, {
  strict: true,
  warningClients: ['gmail', 'apple-mail'], // Only validate for Gmail and Apple Mail
})
```

By default, all four major clients are evaluated: `['outlook', 'gmail', 'apple-mail', 'yahoo']`.
