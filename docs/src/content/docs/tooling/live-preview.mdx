...
<Tabs syncKey="package-manager">
  <TabItem label="npm">```sh npm i -D @hono-email/preview ```</TabItem>
  <TabItem label="yarn">```sh yarn add -D @hono-email/preview ```</TabItem>
  <TabItem label="pnpm">```sh pnpm add -D @hono-email/preview ```</TabItem>
  <TabItem label="bun">```sh bun i -D @hono-email/preview ```</TabItem>
</Tabs>
...
Supported field types include `string`, `number`, `boolean`, `select`, and `array`. See the [API Reference](/api/preview/type-aliases/previewpropsconfig/) for the full schema definition.
