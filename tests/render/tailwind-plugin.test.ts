...
import { buildPerFileArtifactModule, buildPerFileCssModule, transformTailwindComponentSource } from '../../src/unplugin';
...
`;
    ...
    expect(transformed).toContain(`import __EmailTailwindArtifact from 'virtual:hono-email-tw-artifact:${ENCODED_TEST_FILE_ID}'`);
...
`;
  ...
  test('builds a per-file artifact module referencing the per-file CSS virtual module', () => {
    ...
    expect(moduleCode).toContain(`import tailwindCss from 'virtual:hono-email-tw.css:${ENCODED_TEST_FILE_ID}?inline'`);
