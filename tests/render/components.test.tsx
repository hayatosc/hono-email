...
import { Body, Button, Column, Container, Head, Heading, Hr, Html, Link, Preview, Row, Section, Text, render } from '../../src'
...
describe('email components', () => {
  test('renders a basic email document with primitives', async () => {
    ...
    expect(html).toContain('<p style="font-size:14px;line-height:24px;margin-top:16px;margin-bottom:16px">World</p>')
  ...
  test('applies Heading margin shorthand props', async () => {
    const html = await render(
      <Html>
        <Body>
          <Heading as='h3' mx={12} mt='8'>Hello</Heading>
        </Body>
      </Html>
    )

    expect(html).toContain('<h3 style="margin-left:12px;margin-right:12px;margin-top:8px">Hello</h3>')
  })
})
