...
`,
  ...
  test('strips warnings regardless of how many other markers were encoded', () => {
    const first = encodeTailwindWarnings(['warning from render A'])
    for (let i = 0; i < 1200; i += 1) {
      encodeTailwindWarnings([`w${i}`])
    }

    expect(extractTailwindWarnings(`<div>x</div>${first}`)).toEqual({
      html: '<div>x</div>',
      warnings: ['warning from render A'],
    })
  })
