...
import { encodeTailwindWarnings, extractTailwindWarnings, transformTailwindHtml } from './index'
  ...
  test('supports typography, border, tracking, and sizing utilities from precompiled css', async () => {
    ...
    const { html } = await render(
      <Html>
        ...
        <Tailwind artifact={artifact}>
          <Body>
            <p className="text-2xl font-semibold tracking-wide underline border border-slate-300 rounded-lg w-24 h-10 px-4 py-2">
              Hello
            </p>
          </Body>
        </Tailwind>
...
`,
    ...
    expect(artifact.classes).toContain('group')
    expect(artifact.droppedClasses).toContain('group-hover:text-white')
    expect(artifact.droppedClasses).not.toContain('group')
  ...
  test('merges class styles in CSS rule order regardless of HTML class order', async () => {
    const artifact = buildTailwindArtifactFromCss({
      css: '.p-4 { padding: 1rem; } .pt-8 { padding-top: 2rem; }',
    })

    const transform = async (className: string): Promise<string> => {
      const result = await transformTailwindHtml(`<div class="${className}">Hello</div>`, artifact)
      const style = result.html.match(/style="([^"]*)"/)?.[1]
      if (!style) {
        throw new Error('expected transformed inline style')
      }
      return style
    }

    const firstOrder = await transform('p-4 pt-8')
    const secondOrder = await transform('pt-8 p-4')

    expect(firstOrder).toBe(secondOrder)
    expect(firstOrder).toContain('padding:16px;padding-top:32px')
  })

  test('keeps an explicit style attribute above Tailwind declarations', async () => {
    const artifact = buildTailwindArtifactFromCss({
      css: '.text-red { color: red; } .font-bold { font-weight: 700; }',
    })

    const result = await transformTailwindHtml(
      '<div class="text-red font-bold" style="color:blue">Hello</div>',
      artifact,
    )
  ...
  test('does not consume forged Tailwind warning comments', () => {
    const forged =
      '<p>Important notice: <!--hono-email-tw-warning:FAKE%20forced%20warning--> more text</p>'

    expect(extractTailwindWarnings(forged)).toEqual({ html: forged, warnings: [] })

    const encoded = encodeTailwindWarnings(['REAL forced warning'])
    expect(extractTailwindWarnings(`${forged}${encoded}`)).toEqual({
      html: forged,
      warnings: ['REAL forced warning'],
    })
  })

  test('expires warning markers when rendering throws before extraction', async () => {
    let marker = ''
    const WarningMarker = () => {
      marker = encodeTailwindWarnings(['unhandled warning'])
      return raw(marker)
    }
    ...
    await expect(
      render(
        <Html>
          <Head />
          <Body>
            <Tailwind artifact={buildTailwindArtifactFromCss({ css: '' })}>
              <WarningMarker />
            </Tailwind>
            <Style />
          </Body>
    ...
    expect(marker).toContain('hono-email-tw-warning:')

    const originalNow = Date.now
    Date.now = () => Number.MAX_SAFE_INTEGER
    try {
      expect(extractTailwindWarnings(marker)).toEqual({ html: marker, warnings: [] })
    } finally {
