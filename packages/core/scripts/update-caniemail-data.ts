...
const OUTPUT_PATH = resolve(import.meta.dirname, '../src/validate/caniemail-data.ts')
...
const formatTsValue = (value: unknown, depth = 0): string => {
  ...
  if (Array.isArray(value)) {
    return value.length === 0
      ? '[]'
      : `[${value.map((v: unknown) => formatTsValue(v, depth)).join(', ')}]`
  }
  const entries = Object.entries(value)
  if (entries.length === 0) return '{}'
  const lines = entries.map(
    ([k, v]) => `${childPad}${JSON.stringify(k)}: ${formatTsValue(v, depth + 1)}`,
  )
  return `{\n${lines.join(',\n')},\n${pad}}`
}

const generateTsFileContent = (dataFile: DataFile): string =>
  [
    '// Auto-generated by scripts/update-caniemail-data.ts — do not edit manually.',
    `// caniemail.com API ${dataFile.apiVersion} (last updated ${dataFile.lastUpdateDate})`,
    `// Content hash: ${dataFile.contentHash}`,
    '// Run `nr update-caniemail` to regenerate.',
    "import type { ClientDataEntry, ValidationTables } from './caniemail'",
    '',
    `export const tables: ValidationTables = ${formatTsValue(dataFile.tables)}`,
    '',
    `export const clientData: Record<string, ClientDataEntry> = ${formatTsValue(dataFile.clientData)}`,
    '',
  ].join('\n')

const generateDataFile = (apiData: CaniemailApiData): DataFile => {
...
const buildClientData = (
  ...
  for (const entry of CANIEMAIL_FEATURE_MAP) {
    ...
    for (const clientName of EMAIL_CLIENT_LIST) {
      const { client, platform } = EMAIL_CLIENT_PLATFORMS[clientName]
      const platformData = feature.stats[client]?.[platform]
      if (!platformData) {
        continue
      }

      const latestKey = Object.keys(platformData)
...
const main = async (): Promise<void> => {
  ...
  await writeFile(OUTPUT_PATH, generateTsFileContent(dataFile), 'utf8')
